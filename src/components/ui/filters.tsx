...
export function SearchInput({ value, onChange, placeholder, className }: {
  ...
  return (
    <div className={cn('flex h-9 min-w-[240px] flex-1 items-center gap-2 rounded-[10px] border border-line-strong bg-surface px-3', className)}>
      ...
      />
      {text ? (
        <button type="button" aria-label={tr("Clear search")} onClick={() => setText('')} className="text-ink-500 hover:text-ink-900">
          <X className="size-4" />
        </button>
      ) : null}
    </div>
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <label className={cn(controlClass, 'gap-1.5 pr-2')}>
        ...
        >
          {PRESETS.map((item) => (
            <option key={item} value={item}>{tr(presetLabel[item])}</option>
          ))}
    ...
    <div className="flex flex-wrap items-center gap-2">
      {items.map((item) => (
        <span key={item.label} className="inline-flex h-7 items-center gap-1.5 rounded-full bg-brand-50 pl-3 pr-1.5 text-xs font-semibold text-brand-600">
          {item.label}
          <button type="button" aria-label={tr("Remove {0}", { 0: item.label })} onClick={item.onRemove} className="rounded-full p-0.5 hover:bg-brand-100">
            <X className="size-3.5" />
          </button>
        </span>
      ))}
      <Button variant="link" size="sm" onClick={onClear}>{tr("Clear filters")}</Button>
    </div>
  )
}
