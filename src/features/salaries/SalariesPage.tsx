...
export default function SalariesPage() {
  ...
  const columns: Column<SalaryOverviewEmployee>[] = [
    ...
    { key: 'salary', header: 'Monthly salary', align: 'right', priority: 2, mobile: 'meta', cell: (row) => <span className="num font-semibold text-ink-900">{formatMoney(money(row.monthly_salary, currency.decimals), currency)}</span> },
    { key: 'paid', header: 'Paid', align: 'right', priority: 2, mobile: 'meta', cell: (row) => <span className="num text-ink-600">{formatMoney(money(row.paid, currency.decimals), currency)}</span> },
    {
      key: 'remaining',
      header: 'Remaining',
      align: 'right',
      ...
      cell: (row) => {
        ...
        return (
          <div className="flex flex-col gap-1">
            ...
            <span className="num text-xs text-ink-500">{Math.round(percent)}% paid</span>
          </div>
        )
      },
    },
    { key: 'state', header: 'Status', priority: 1, mobile: 'meta', cell: (row) => <StatusBadge status={row.state} /> },
    {
      key: 'last',
      header: 'Last payment',
      priority: 3,
    ...
    },
    {
      ...
      cell: (row) =>
        can('salaries.pay') ? (
          row.state === 'paid' ? (
            <Button size="sm" disabled title={`Fully paid for ${formatMonth(period)} — record an adjustment to pay more`}>
              Paid
            </Button>
          ) : (
  ...
  return (
    <>
      <PageHeader
        title="Salaries"
        subtitle="Mushahar · Monthly salary obligations and payments"
        actions={
          <>
            <div className="flex items-center gap-1 rounded-xl border border-line-strong bg-surface p-0.5">
              <Button variant="ghost" size="icon" aria-label="Previous month" onClick={() => setPeriod(shiftMonth(period, -1))}>
      ...
      {tab === 'overview' ? (
        <>
            ...
            {overview.isLoading ? (
              ...
              <>
                <StatCard
                  ...
                  tone="brand"
                  caption={`${totals?.employee_count ?? 0} employees in ${formatMonth(period)}`}
                />
                <StatCard
                  label="Paid so far"
                  ...
                  caption={
                    totals && totals.obligations > 0
                      ? `${Math.round((totals.paid / totals.obligations) * 100)}% of this month`
                      : 'Nothing due yet'
                  }
                  footer={
                    totals && totals.obligations > 0 ? (
                ...
                <StatCard
                  ...
                  tone="pending"
                  caption={`${totals?.unpaid_count ?? 0} employees not fully paid`}
                />
                <StatCard
                  label="Arrears"
                  ...
                  tone="neutral"
                  caption={arrears > 0 ? 'Unpaid from earlier months' : 'Earlier months are fully paid'}
                />
              </>
            )}
          </div>
          ...
          <Card className="overflow-hidden">
            {overview.isError ? (
              <ErrorState message="Could not load salaries" onRetry={() => void overview.refetch()} />
            ) : (
              <DataTable
                columns={columns}
                rows={employees}
                ...
                caption={`Salaries for ${formatMonth(period)}`}
                empty={
                  <EmptyState
                    icon={<Briefcase className="size-6" />}
                    title="No employees for this month"
                    description="Add your trainers and staff to track their monthly salaries."
                    actions={<Link to="/employees"><Button variant="primary">Go to employees</Button></Link>}
                  />
                }
              />
            )}
        ...
        <Card className="overflow-hidden">
          {history.isLoading ? (
            ...
            <DataTable
              ...
              keyOf={(row) => row.id}
              caption="Salary payment history"
              columns={[
                { key: 'date', header: 'Paid on', priority: 1, mobile: 'meta', cell: (row) => <span className="whitespace-nowrap">{formatBusinessDate(row.business_date)}</span> },
                {
                  key: 'employee',
                  header: 'Employee',
                  priority: 1,
                ...
                { key: 'period', header: 'Period', priority: 2, mobile: 'meta', cell: (row) => formatMonth(row.period_month) },
                { key: 'type', header: 'Type', priority: 3, mobile: 'hide', cell: (row) => <StatusBadge status={row.payment_type} /> },
                { key: 'method', header: 'Method', priority: 3, mobile: 'hide', cell: (row) => row.payment_method_name },
              ...
              empty={<EmptyState icon={<BadgeDollarSign className="size-6" />} title="No salary payments yet" description="Salary payments appear here once you pay someone." />}
            />
          )}
        </Card>
      )}
...
function PaySalaryDialog({ employee, employees, period, onClose }: {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      toast.success(`Salary of ${formatMoney(money(result.amount, currency.decimals), currency)} paid`, {
        description: `Recorded as one expense · TX-${String(result.reference_no).padStart(6, '0')}`,
      })
  ...
  return (
    <Modal
      ...
      title={step === 'details' ? 'Pay salary' : 'Review salary payment'}
      description={step === 'details' ? `Mushahar · ${formatMonth(period)}` : 'Check the details before the money is recorded'}
      ...
      footer={
        step === 'details' ? (
            ...
            <Button
              ...
              onClick={() => {
                if (!employeeId || moneyError(amount, currency) || !methodId) {
                  toast.info('Check the payment details', 'Employee, amount and payment method are required.')
                  return
                }
                if (type === 'adjustment' && reason.trim().length < 5) {
                  toast.info('Give a reason for the adjustment', 'At least 5 characters.')
                  return
                }
                setStep('review')
              }}
          ...
          <>
            <Button icon={<ArrowLeft className="size-4" />} onClick={() => setStep('details')}>Back</Button>
            <Button variant="primary" loading={mutation.isPending} onClick={() => mutation.mutate()}>Confirm payment</Button>
          </>
        )
      }
    >
      {step === 'details' ? (
        <div className="flex flex-col gap-4">
            ...
            <Select value={employeeId} onChange={(event) => setEmployeeId(event.target.value)}>
              {employees.map((item) => (
                <option key={item.employee_id} value={item.employee_id}>
                  {item.full_name} — {formatMoney(money(item.remaining, currency.decimals), currency)} left
                </option>
              ))}
            </Select>
          </Field>
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Payment type">
              <Select value={type} onChange={(event) => setType(event.target.value as typeof type)}>
                ...
                <option value="adjustment">Adjustment (extra)</option>
              </Select>
            </Field>
            <Field label="Amount" error={amount ? moneyError(amount, currency) : undefined} hint={`Remaining for ${formatMonth(period)}: ${formatMoney(remaining, currency)}`}>
          ...
          {type === 'adjustment' ? (
            <Field label="Reason for the adjustment">
              <Textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="e.g. Bonus for covering extra shifts" />
            </Field>
          ) : null}
          ...
          <div className="flex items-center gap-3.5 rounded-xl border border-line bg-surface-2 p-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-sm text-ink-500">
                {selected?.position} · monthly salary {formatMoney(money(selected?.monthly_salary ?? 0, currency.decimals), currency)}
              </p>
            </div>
            <div className="text-right">
              <p className="text-xs text-ink-500">Paying</p>
              <p className="num text-2xl font-bold text-ink-900">{amount}</p>
            </div>
          </div>
          <dl className="grid grid-cols-[150px_minmax(0,1fr)] gap-y-2.5 text-sm">
            <dt className="text-ink-500">Salary period</dt><dd className="font-medium">{formatMonth(period)}</dd>
            <dt className="text-ink-500">Payment type</dt><dd className="font-medium capitalize">{type}</dd>
            <dt className="text-ink-500">Paid from</dt><dd className="font-medium">{activeMethods.find((m) => m.id === methodId)?.name}</dd>
            <dt className="text-ink-500">Payment date</dt><dd className="font-medium">{formatBusinessDate(date)}</dd>
          </dl>
          <Callout tone="info">
            This records <strong>one expense</strong> in the <strong>Salary</strong> category, dated {formatBusinessDate(date)}.
            It is counted once — in Expenses, Salaries and Reports.
          </Callout>
        </div>
      )}
    </Modal>
