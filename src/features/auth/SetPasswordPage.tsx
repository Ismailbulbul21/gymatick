  ...
  return (
    <div className="grid min-h-svh place-items-center bg-canvas p-6">
      <Card className="w-full max-w-[440px] p-8">
        ...
        <h1 className="mt-6 text-2xl font-bold text-ink-900">Choose your own password</h1>
        <p className="mt-1 text-sm text-ink-500">
          {context?.user.full_name ? `${context.user.full_name}, y` : 'Y'}ou are signed in with a temporary password.
          Set your own before you start recording money.
        </p>
        <Callout tone="info" className="mt-5" icon={<KeyRound className="size-[18px] text-brand-600" />}>
          Nobody else should know this password — every entry you record is stored with your name.
        </Callout>
        <PasswordForm
