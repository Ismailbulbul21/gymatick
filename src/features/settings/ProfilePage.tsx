...
export default function ProfilePage() {
  ...
  const [language, setLanguage] = useState<'en' | 'so'>(user?.preferred_language ?? 'en')
  ...
  return (
    <>
      <Card>
        <CardHeader title="My profile" description="Your name is stored with every record you create" />
        <div className="flex flex-col gap-4 p-5 pt-4">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-xs text-ink-500">
                {membership?.role === 'owner' ? 'Admin' : (membership?.title ?? 'Shaqaale')} · {membership?.business_name}
              </p>
            </div>
          </div>
          ...
          <div className="flex justify-end">
            <Button
              ...
              onClick={() => {
                if (!fullName.trim()) return toast.info('Enter your name')
                mutation.mutate()
              }}
            >
              Save profile
      ...
      <Card>
        <CardHeader title="Password" description="At least 10 characters, with upper and lower case letters and a number" />
        <div className="p-5 pt-4">
          <Callout tone="info" icon={<KeyRound className="size-[18px] text-brand-600" />}>
            Keep your password to yourself — every entry you record carries your name.
          </Callout>
